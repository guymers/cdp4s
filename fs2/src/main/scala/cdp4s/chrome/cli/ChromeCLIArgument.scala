...
abstract class ChromeCLIArgument(val name: String, value: => Option[String]) extends Product with Serializable {
  val flag: String = show"$name${value.map(v => show"=$v").getOrElse("").show}"
...
object ChromeCLIArgument {
  ...
  case object DisableBackgroundingOccludedWindows
    extends ChromeCLIArgument("--disable-backgrounding-occluded-windows", None)
  case object DisableBreakpad extends ChromeCLIArgument("--disable-breakpad", None)
  case object DisableClientSidePhishingDetection
    extends ChromeCLIArgument("--disable-client-side-phishing-detection", None)
  ...
  final case class RemoteDebuggingPort(port: Int)
    extends ChromeCLIArgument("--remote-debugging-port", Some(port.toString))
  ...
  final case class UserDataDir(directory: Path)
    extends ChromeCLIArgument("--user-data-dir", Some(directory.toFile.getAbsolutePath))
