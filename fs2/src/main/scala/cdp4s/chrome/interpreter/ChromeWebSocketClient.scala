...
object ChromeWebSocketClient {
  ...
  def connect[F[_]](
    ...
    new ChromeWebSocketClient[F] {
      ...
      override def runCommand[T: Decoder](method: String, params: JsonObject, sessionId: Option[SessionID]): F[T] =
        for {
      ...
      private def _runCommand[T: Decoder](method: String, params: JsonObject, sessionId: Option[SessionID]): F[T] =
        for {
  ...
  private def createHandler[F[_]](
    ...
    val out = fs2.Stream.fromQueueNoneTerminated(requestQ).evalMap { request =>
          ...
          jsonObj.asJson.as[ChromeResponse[JsonObject]]
            ...
            .flatMap { case (id, result) =>
              for {
                ...
                _: Boolean <- callback.complete(result)
