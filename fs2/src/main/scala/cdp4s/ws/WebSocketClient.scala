...
object WebSocketClient {
  ...
  def create[F[_]](uri: WsUri, options: Options)(implicit F: Async[F], R: Runtime[F]): Resource[F, WebSocketClient[F]] = {
    ...
    for {
      ...
      _ <- fs2.Stream.fromQueueUnterminated(messageQueue).evalTap {
  ...
  private def listener[F[_]](
  ...
  )(implicit F: Functor[F], R: Runtime[F]) = new WebSocket.Listener {
    ...
    private def offer(msg: WebSocketListenerMessage) = R.unsafeRun {
      q.offer(msg).as(())
    }

    private def offerSync(msg: WebSocketListenerMessage) = R.unsafeRunSync {
      q.offer(msg).as(())
    }
